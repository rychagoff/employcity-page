...
Для работы с репозиторием требуется _Git_ и _Node.js_ версии не ниже 18.
...
Все HTML-файлы с разметкой складывать в папку `source/`.
...
Все БЭМ-блоки и остальные препроцессорные файлы подключать в `source/styles/styles.scss`.
...
БЭМ-блоки импортировать в секцию `/* BLOCKS */`.
...
Абсолютно всю растровую графику с двухкратной плотностью из макета складывать в `raw/images/`. Здесь графика игнорируется гитом.

После добавления графики нужно запустить команду `npm run optimize:raster` (или просто `npm run optimize`) для оптимизации графики и создания `.webp`-версии. Команду нужно запускать один раз при появлении новой графики в проекте.
...
Контентную векторную графику (логотип, графики, иллюстрации) можно тоже складывать в `raw/images/`. Запуск команды `npm run optimize:vector` (или просто `npm run optimize`) поместит оптимизированные копии этих svg-файлов в `source/images/`
...
Векторную графику для спрайта (иконки) складывать в `raw/images/icons/`.
